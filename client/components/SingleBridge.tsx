...
export default function SingleBridge(){
  ...
  const {
    ...
    error,
    isLoading,
  } = useQuery({ queryKey: ['bridge', params.id], queryFn: () => getBridge(Number(params.id)) })
  ...
  return <main>
    <Header>
      <div className="header-left flex flex-col gap-3">
        <h2 className="font-bold text-2xl">{bridge.name}</h2>
        <div>
          ...
          <div className="flex gap-3">
            ...
            {<p>{bridge.addedByUser ?? <span style={{fontStyle: "italic"}}>None</span>}</p>}
          </div>
        </div>
      </div>
